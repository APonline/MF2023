...
export class LyricsFormComponent implements OnInit, OnChanges {
  ...
  setSettings(formData){
    ...
    newForm['owner_album'] = '';
    this.displayedColumns.push('owner_song');
    form['owner_song'] = new FormControl('');
    newForm['owner_song'] = '';
    this.displayedColumns.push('title');
    form['title'] = new FormControl('');
    newForm['title'] = '';
    this.displayedColumns.push('lyrics');
    form['lyrics'] = new FormControl('');
    newForm['lyrics'] = '';
  ...
  openDialog(action: string, row: any) {
    ...
    this.MF
      ...
      .subscribe(result => {
        ...
        // post-process fields for this tool
        const cooked = this.MF
          .compose(result.data)
          .with({
            profile_url: `${this.artist?.profile_url}-${(result.data?.title ?? '')
              .toString()
              .replace(/[^\w\s-]/g, '')   // drop punctuation (safer)
              .replace(/\s+/g, '')        // remove spaces (use '-' if you prefer)
              .toLowerCase()}`,
